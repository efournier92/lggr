class Week
  ...
  def self.days
    task = Task.new
    { 
      "Monday"    => [task.amazon_fresh_recieve, task.pch_spectrum, task.vitamins_take, task.excercise, task.apartment_kitchen_all_clean, task.aquarium_jar_and_tank_water_change, task.git, task.music, task.nobo],
      ...
      "Sunday"    => [task.git, task.call_mom_and_dad, task.aquarium_jar_and_tank_water_change_with_filter, task.juice_make, task.apartment_all, task.docs_scan, task.screens_clean, task.groom_all, task.lg_all, task.music, task.media_consume]
    }
  end
